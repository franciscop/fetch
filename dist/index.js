...
const hasObjectBody = (body) => {
    ...
    if (body instanceof FormData)
        return false;
    if (typeof (body.pipe || body.pipeTo) === "function")
        return false;
...
const createUrl = (url, query, base) => {
    ...
    const entries = new URLSearchParams(Object.fromEntries([
    ...
    if (!base)
        return path;
    const fullUrl = new URL(path.replace(/^\//, ""), base.replace(/\/$/, "") + "/");
    return fullUrl.href;
};
...
function create(defaults = {}) {
    ...
    const fch = swear(async (url = "/", options = {}) => {
        ...
        before, after, error, cache, ...request } = { ...fch, ...options }; // Local option OR global value (including defaults)
        // Absolute URL if possible; Default method; merge the default headers
        request.url = createUrl(url, { ...fch.query, ...options.query }, request.baseUrl ?? request.baseURL);
        ...
        request = before ? await before(request) : request;
